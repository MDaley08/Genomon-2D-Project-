...
typedef struct
{
    TextLine    name; //name of level
    Sprite     *tileSet; //texture to extra frames from to draw tile layer
    Vector2D    tileSize;    //the width and height of the tile map
    Vector2D    mapSize;    //the width and height of the tile map
    int        *tileMap;    //array containing numbers that correspond to a texture in tileset
    Sprite     *tileLayer;  //sprite created from tileset and tilemap
    List       *clips;      //list of static shapes to clip against in the game world
}Level;

/**
 * @brief get the currently active level
 ...
 * @return NULL on not found or other error, or returns the loaded level
 */
